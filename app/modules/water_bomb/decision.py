...
if __name__ == '__main__':
    ...
    bullet = 1
    reversal = False
    sitems = ['gem_of_life', 'reset_hammer']
    eitems = ['reverse_magic', 'reverse_magic', 'reverse_magic', 'reverse_magic', 'insight_sunglasses']
    print(f'我方hp: {shp}')
    print(f'敌方hp: {ehp}')
    print(f'实弹: {live}')
    print(f'虚弹: {blank}')
    print(f'伤害反转：{reversal}')
    print(f'看到当前子弹的类型: {bullet}')
    print(f'我方当前道具：{sitems}')
    print(f'敌方当前道具：{eitems}')
    status = Status(
        maxhp=maxhp,
        shp=shp,
        ehp=ehp,
    ...
    round = Round()
    win_prob, strategy = round.optimal_strategy(status)
    print(f'当前最佳操作为：{strategy}, 胜率为：{win_prob}')
    end = time()
    print(end - start)