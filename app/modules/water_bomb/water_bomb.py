...
class WaterBombModule(BaseTask):
    def __init__(self):
        ...
        self.normal_items = ['advanced_barrel','gem_of_life','handcuffs','reverse_magic','insight_sunglasses','unload_puppet']
    ...
    def enter_and_start(self):
        ...
        while True:
            ...
            if self.auto.click_element('最佳记录','text',crop=(1279/1920,619/1080,1382/1920,658/1080)):
                continue
            if self.auto.find_element(['心动水弹','F'],'text',crop=(1025/1920,663/1080,1267/1920,717/1080)):
    ...
    def fight(self):
        ...
        while True:
            ...
            # 确认加速后进入主循环
                ...
                # 检查是否进入自己的回合
                if self.auto.find_element('app/resource/images/water_bomb/shoot_self.png', 'image',crop=(1487/1920,815/1080,1723/1920,900/1080),match_method=cv2.TM_CCOEFF_NORMED):
                ...
                if is_player_round :
                    if current_strategy == '':
                        ...
                        if not is_open_items:
                            self.auto.click_element('查看道具', 'text', crop=(113 / 1920, 984 / 1080, 220 / 1920, 1020 / 1080))
                            time.sleep(0.5)
                            continue
                        # 更新状态
                        ...
                        if win_prob == 0:
                            self.restart()
                            break
                        if win_prob <0.4 and 'reset_hammer' in self.player_items:
        ...
        while True:
            ...
            # 射击敌人
                ...
                if self.auto.find_element('回合转换','text',crop=(872/1920,477/1080,1120/1920,550/1080)):
                    ...
                    self.current_power = 1
                    return True
                if self.have_extra_shoot and self.auto.find_element('回合追加','text',crop=(872/1920,477/1080,1120/1920,550/1080)):
                    self.have_extra_shoot = False
                    self.sustain = False
                    self.is_reversal = False
                    self.current_power = 1
                    return True
                if self.auto.find_element('app/resource/images/water_bomb/gun.png','image',crop=(1494/1920,352/1080,1714/1920,581/1080),match_method=cv2.TM_CCOEFF_NORMED):
    ...
    def handle_insight_sunglasses(self):
        ...
        while True:
            ...
            crop_image = self.auto.get_crop_form_first_screenshot(crop=(922/1920,972/1080,1227/1920,1021/1080))
    ...
    def select_and_reset(self):
        ...
        while True:
            ...
            if self.auto.find_element('查看道具', 'text', crop=(113 / 1920, 984 / 1080, 220 / 1920, 1020 / 1080)):
                return True
            if self.auto.find_image_and_count(self.auto.get_crop_form_first_screenshot(crop=(440 / 1920, 332 / 1080, 1510 / 1920, 878 / 1080)),'app/resource/images/water_bomb/steal_select.png') == len(items_list):
                select_flag = True
            else:
                select_flag = False
            if select_flag and not self.auto.find_element('确定','text',crop=(887/1920,821/1080,967/1920,863/1080)):
                finish_flag = True
            if finish_flag and not self.auto.find_element('查看道具', 'text', crop=(113 / 1920, 984 / 1080, 220 / 1920, 1020 / 1080)):
                self.auto.click_element('关闭','text',crop=(887 / 1920, 821 / 1080, 967 / 1920, 863 / 1080))
                continue
            if not select_flag:
                if top_left and bottom_right:
                    if self.auto.find_element('app/resource/images/water_bomb/steal_select.png','image',crop=(top_left[0]/1920,top_left[1]/1080,bottom_right[0]/1920,bottom_right[1]/1080),match_method=cv2.TM_CCOEFF_NORMED):
                        index += 1
                        if index >= len(items_list):
                            return True
                path = f'app/resource/images/water_bomb/{items_list[index]}_steal.png'
                result = self.auto.find_element(path,'image',crop=(440 / 1920, 332 / 1080, 1510 / 1920, 878 / 1080),match_method=cv2.TM_CCOEFF_NORMED)
    ...
    def select_and_steal(self,steal_item):
        ...
        while True:
            ...
            if self.auto.find_element('请选择','text',crop=(804/1920,516/1080,1125/1920,560/1080)):
                select_flag = False
            if self.auto.find_element('app/resource/images/water_bomb/steal_select.png','image',crop=(440 / 1920, 332 / 1080, 1510 / 1920, 878 / 1080),
    ...
    def use_normal_item(self,item_name):
        ...
        while True:
            ...
            if appear_use_button and not self.auto.find_element('使用', 'text', crop=(500 / 1920, 700 / 1080, 562 / 1920, 735 / 1080)):
                time.sleep(1)
                return True
            if self.auto.find_element('使用', 'text', crop=(500 / 1920, 700 / 1080, 562 / 1920, 735 / 1080)):
                appear_use_button = True
            if not appear_use_button:
                self.auto.click_element(path, 'image', crop=(145 / 1920, 758 / 1080, 665 / 1920, 852 / 1080),match_method=cv2.TM_CCOEFF_NORMED)
                time.sleep(0.5)
                continue
            else:
    ...
    def update_hp_and_bullet(self):
        ...
        player_hp_screenshot = self.auto.get_crop_form_first_screenshot((683/1920,131/1080,942/1920,186/1080))
        enemy_hp_screenshot = self.auto.get_crop_form_first_screenshot((943/1920,131/1080,1237/1920,186/1080))
        self.current_player_hp = self.auto.find_image_and_count(player_hp_screenshot,'app/resource/images/water_bomb/hp.png')
        ...
        self.current_enemy_hp = self.auto.find_image_and_count(enemy_hp_screenshot,'app/resource/images/water_bomb/hp.png')
